...
echo "<h3>1. Цикл do-while (маркированный список)</h3><ul>";
$i = 0;
do {
...
echo "<h3>2. Цикл for (нумерованный список)</h3><ol>";
for ($j = 0; $j < count($track); $j++) {
    echo "<li>(" . $track[$j]['id_track'] . ") " . $track[$j]['name'] . " (id альбома: " . $track[$j]['id_album'] . ")</li>";
}
echo "</ol><hr>";

echo "<h3>3. Цикл foreach (таблица)</h3><table border='1' cellpadding='5'>";
echo "<tr><th>id</th><th>Трек</th><th>Примечание</th><th>Id-альбома</th></tr>";
foreach ($track as $row) {
    echo "<tr><td>{$row['id_track']}</td><td>{$row['name']}</td><td>{$row['note']}</td><td>{$row['id_album']}</td></tr>";
}
echo "</table><hr>";

echo "<h3>4. Цикл while (таблица)</h3><table border='1' cellpadding='5'>";
echo "<tr><th>id</th><th>Трек</th><th>Примечание</th><th>Id-альбома</th></tr>";
$k = 0;
while ($k < count($track)) {
    echo "<tr><td>" . $track[$k]['id_track'] . "</td><td>" . $track[$k]['name'] . "</td><td>" . $track[$k]['note'] . "</td><td>" . $track[$k]['id_album'] . "</td></tr>";
    $k++;
}
echo "</table>";
?>
